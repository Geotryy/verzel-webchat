...
export async function getLeadBySessionId(sessionId: string): Promise<Lead | undefined> {
  const db = await getDb();
...
export async function updateLead(sessionId: string, data: Partial<InsertLead>): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function updateConversationStatus(sessionId: string, status: "active" | "completed" | "abandoned"): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.update(conversations).set({ status }).where(eq(conversations.sessionId, sessionId));
}
